...
- [x] **PDF-парсер и ZIP-инжест выписок (итерация 2b)** — текстовые PDF (извлечение таблиц + запасной построчный разбор «дата … сумма»); загрузка ZIP-архива, где каждый вложенный файл (CSV/XLSX/PDF/1С) разбирается своим парсером, операции объединяются и проходят общую дедупликацию (кросс-форматную: одна и та же операция из PDF и CSV не задваивается)
...
│       ├── api.py               # REST API
│       ├── parsers/             # выписки: csv, xlsx, onec (1С), pdf; + ZIP-инжест (parse_archive в __init__)
│       ├── categorization/
...
curl -F "file=@../samples/statement_demo.pdf" localhost:8000/api/imports/upload
curl -F "file=@../samples/statements_demo.zip" localhost:8000/api/imports/upload
```
...
| `POST` | `/api/imports/upload` | Загрузка выписки (multipart `file`, форматы CSV/XLSX/PDF/1С/ZIP): парсинг → дедупликация → категоризация |
...
4. **PDF** — выписка с текстовым слоем. Табличные PDF разбираются извлечением таблиц (pdfplumber) и тем же движком распознавания колонок, что CSV/XLSX; для простых макетов есть запасной построчный разбор «дата … сумма» с определением направления по знаку суммы. Отсканированные PDF без текстового слоя не поддерживаются — выдаётся понятная ошибка с предложением включить OCR или загрузить CSV/XLSX.
5. **ZIP-архив** — пакетная загрузка: архив распаковывается, каждый вложенный файл (CSV / XLSX / PDF / 1С) разбирается своим парсером, операции объединяются, дубликаты отсекаются общей дедупликацией. Вложенные архивы и «зип-бомбы» отсекаются лимитами (число файлов, размер файла и суммарный размер).
