...
- [x] **Google Sheets — двусторонняя синхронизация (первый срез)** — OAuth-подключение аккаунта; `push` собирает полноформатный .xlsx (те же формулы и именованные диапазоны) и загружает его в Google Drive с конвертацией в нативную таблицу; `pull` читает ключевые драйверы-входы с листа «Допущения» обратно в профиль. Повторный `push` обновляет ту же таблицу (ссылка не меняется). Полное отображение каждой ячейки модели в оба конца — следующий шаг.
...
| `GOOGLE_CLIENT_ID` | _(пусто)_ | OAuth client id (Google Cloud). Без него синхронизация с Google Sheets выключена |
| `GOOGLE_CLIENT_SECRET` | _(пусто)_ | OAuth client secret |
| `GOOGLE_REDIRECT_URI` | `http://localhost:8000/api/google/callback` | Redirect URI OAuth; должен совпадать с настройкой в Google Cloud и адресом деплоя |
...
| `GET` | `/api/google/status` | Статус интеграции Google: настроена ли, подключён ли аккаунт, ссылка на таблицу |
| `GET` | `/api/google/auth` | Перенаправление на экран согласия Google (OAuth) |
| `GET` | `/api/google/callback` | Обратный вызов OAuth: обмен кода на токены |
| `POST` | `/api/google/disconnect` | Отключение Google-аккаунта (удаление токенов) |
| `POST` | `/api/sheets/push` | Создать/обновить Google-таблицу модели из профиля (параметр `title`) |
| `POST` | `/api/sheets/pull` | Прочитать драйверы-входы из таблицы обратно в профиль |
...
1. **Google Sheets** — модель живёт в таблице пользователя: создание из шаблона, чтение и запись через Sheets API. _Первый срез готов_ (см. выше и `docs/google_sheets_setup.md`): OAuth, `push` (xlsx → конвертация в Google-таблицу), `pull` драйверов-входов. Дальше — полный двусторонний маппинг всех ячеек и автосинхронизация по расписанию.
