...
- **Model book (v1)** — sheet-registry engine: series, metrics, scenario support, Excel export per registry; 15 sheets — the full v1 target architecture — including Roadmap, Production/procurement (unit costs, logistics, storage, lead-time purchases with inventory on the balance sheet), Sales plan, CAPEX & depreciation, Opex breakdown, Headcount & payroll (with social contributions), monthly P&L with profit-tax schedules, forecast Balance sheet (assets = liabilities identity), Covenants (DSCR / ICR / Debt-to-EBITDA vs thresholds — breaches also surface as validator open questions), and an NPV Sensitivity matrix (price × volume, color-scaled); named cells and ranges (RevenueRow, NetCFRow, NPV, DSCRmin…) form the AFM&C contract for AI agents working directly in Excel — all export paths now serve the registry book (the legacy export engine is fully retired; `/api/model/export` builds an assumption set from the survey or the default profile when none exists)
...
- [x] **Рабочий стол (новый UX)** — интерфейс в духе AI-платформ: по центру диалоговый композер с голосовым вводом (Web Speech) и загрузкой документов перетаскиванием; ответом разворачивается финмодель как редактируемая таблица (статьи × месяцы) — факт из выписки заблокирован, допущения и пустые ячейки правятся прямо в сетке с сохранением в БД (`PUT /api/assumptions`); снизу — док загруженных документов; переключение светлой/тёмной темы; кнопки экспорта в .xlsx и синхронизации с Google Sheets. Прежний интерфейс сохранён на `/legacy`.
...
- [x] **Книга модели (v1)** — движок по реестру листов: ряды, метрики, сценарии, экспорт в Excel по реестру; 15 листов — целевая архитектура v1 полностью: Обложка, Дорожная карта (вехи + расчётные вехи из движка), Допущения, CF (после налога), Дашборд, План продаж, Производство (себестоимость, логистика, хранение, опережающие закупки → запасы в балансе), CAPEX и амортизация, Опер. расходы, ФОТ (штатное расписание со взносами), ПиУ, Балансы (прогнозный баланс с тождеством Активы = Пассивы), Кредит, Ковенанты (DSCR / ICR / Долг-EBITDA против порогов; нарушения попадают и в открытые вопросы валидатора), Чувствительность (матрица NPV цена × объём с цветовой шкалой); именованные ячейки и ряды (RevenueRow, NetCFRow, NPV, DSCRmin…) — контракт AFM&C для ИИ-агентов в Excel; все пути экспорта отдают книгу по реестру — legacy-движок экспорта полностью выведен из эксплуатации (`/api/model/export` при отсутствии набора собирает его из опросника или auto-профиля и сохраняет версией)
